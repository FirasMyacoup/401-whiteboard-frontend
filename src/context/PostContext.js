...
const PostContextProvider = (props) => {
 ...
 const editPost = async (e, id) => {
  ...
  swal({
   ...
   if (willedit) {
    ...
    await axios
     .put(`${process.env.REACT_APP_HEROKU_URL}/post/${id}`, post, {
      headers: {
       Authorization: `bearer ${cookies.load("token")}`,
      },
     })
 ...
 const deletePost = async (id) => {
  swal({
   title: "please not if you delete this you wont be able to recover it",
   icon: "warning",
   buttons: true,
   dangerMode: true,
   ...
   if (willDelete) {
    await axios
     .delete(`${process.env.REACT_APP_HEROKU_URL}/post/${id}`, {
      headers: {
       Authorization: `bearer ${cookies.load("token")}`,
      },
     })
